...
function drawLine(x1, y1, x2, y2) {
  if (
    canvasSize.width >= Math.max(x1, x2) &&
    canvasSize.height >= Math.max(y1, y2) &&
    x1 &&
    x2 &&
    y1 &&
    y2
...
fs.readFile("input.txt", "utf8", (err, contents) => {
  ...
  for (let i = canvasIndex + 1; i < text.length; i++) {
    switch (text[i][0]) {
      case "L": {
        drawLine(+text[i][1], +text[i][2], +text[i][3], +text[i][4]);
        appendToFile();
        break;
      }
      case "R": {
        drawRectangle(+text[i][1], +text[i][2], +text[i][3], +text[i][4]);
        appendToFile();
        break;
      }
      case "B": {
        fillBucket(+text[i][2], +text[i][1], " ", text[i][3]);
        appendToFile();
        break;
      }
      default:
